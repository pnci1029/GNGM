...
### 보안 및 설정 관리 원칙 (필수)
- **환경변수 전용**: 모든 설정 정보는 process.env에서만 가져오기
- **하드코딩 절대 금지**: `||` 연산자 기본값 노출 완전 차단
- **필수 검증**: 환경변수 누락 시 애플리케이션 시작 실패 구현
- **시크릿 분리**: playground 디렉토리에서 Git 외부 관리
- **타입 안전성**: parseInt(), parseFloat() 등으로 타입 변환

**상세 내용**: `/docs/api-standards.md`, `/docs/secrets-management.md` 참조
